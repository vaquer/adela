...
feature "data catalog management" do
  ...
  scenario "can't consume unpublished catalog data" do
    given_there_is_an_inventary_by("CONEVAL")
    get "/coneval/catalogo.json"
    gets_empty response
  end

  def given_there_is_a_catalog_published(days_ago)
    @inventory = FactoryGirl.create(:inventory, :published => true, :publish_date => days_ago)
  ...
  def given_there_is_an_inventary_by(organization_title)
