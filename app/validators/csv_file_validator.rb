class CsvFileValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    if value.blank?
      record.errors[:base] << I18n.t("activerecord.errors.models.catalog.attributes.csv_file.blank_file")
    else
  ...
  def message(error)
    I18n.t("activerecord.errors.models.catalog.attributes.csv_file.#{error.type.to_s}", row: error.row)
  end
end
